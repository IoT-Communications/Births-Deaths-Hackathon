...
class BirthRegistration(Document):
	...
	def on_update(self):
		...
		if self.workflow_state == "Ready for Payment":
			...
			#frappe.db.set_value("Birth Registration", self.name, "ready_for_payment", 1)

        
		#if self.workflow_state == "Approved" and self.approved == 0:
			#frappe.db.set_value("Birth Registration", self.name, "approved", 1)
	...
	def on_payment_authorized(self, status=None):
		...
		if status in ["Authorized", "Completed"]:
			...
			if doc.workflow_state == "Pending Payment":
				...
				redirect_to = get_url("/app/birth-registration/" + self.name)
				return redirect_to
